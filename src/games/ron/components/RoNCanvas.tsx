...
export function RoNCanvas({ navigationTarget, onNavigationComplete, onViewportChange }: RoNCanvasProps) {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      // Draw enhanced realistic sky background
      drawRealisticSky(ctx, canvas, 'day', fireAnimTimeRef.current);
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          if (tile.terrain === 'water') {
            ...
            // Use enhanced photorealistic water rendering
            drawRealisticWaterTile(ctx, screenX, screenY, x, y, fireAnimTimeRef.current, currentZoom, adjacentWater, { ambient: 1.0, sparkle: true });
            
            // Draw fishing spot indicator with enhanced rendering
            if (tile.hasFishingSpot) {
              drawFishingSpot(ctx, screenX, screenY, x, y, fireAnimTimeRef.current);
            }
            ...
            if (tile.hasMetalDeposit) {
              // Draw enhanced photorealistic mountain terrain
              drawRealisticMountain(ctx, screenX, screenY, x, y, true, currentZoom);
            } else if (tile.hasOilDeposit) {
              // Draw grass base first with enhanced rendering
              drawRealisticGrassTile(ctx, screenX, screenY, x, y, currentZoom, { ambient: 1.0 });
              
              // Only show oil in industrial+ ages
              const isIndustrial = AGE_ORDER.indexOf(playerAge) >= AGE_ORDER.indexOf('industrial');
              if (isIndustrial) {
                drawRealisticOilDeposit(ctx, screenX, screenY, x, y, currentZoom);
              }
            } else if (tile.forestDensity > 0) {
              // Draw enhanced grass base tile for forest
              drawRealisticGrassTile(ctx, screenX, screenY, x, y, currentZoom, { ambient: 1.0 });
              ...
              // Regular grass tile with enhanced photorealistic rendering
              drawRealisticGrassTile(ctx, screenX, screenY, x, y, currentZoom, { ambient: 1.0 });
            }
      ...
      // Beach pass: Draw enhanced photorealistic beaches on water tiles adjacent to land
      ...
      for (let y = 0; y < gameState.gridSize; y++) {
        for (let x = 0; x < gameState.gridSize; x++) {
          ...
          // Draw enhanced photorealistic beach if any adjacent tile is land (and not a dock)
          if (adjacentLand.north || adjacentLand.east || adjacentLand.south || adjacentLand.west) {
            drawRealisticBeach(ctx, screenX, screenY, x, y, adjacentLand, currentZoom, fireAnimTimeRef.current);
          }
        }
      }
      ...
      // Fourth pass: Draw units with pedestrian-like sprites and shadows
      gameState.units.forEach(unit => {
        ...
        // Draw unit shadow for depth perception
        const unitCenterX = screenX + TILE_WIDTH / 2;
        const unitCenterY = screenY + TILE_HEIGHT / 2;
        drawUnitShadow(ctx, unitCenterX, unitCenterY, 4 * currentZoom, 0);
        
        // Draw unit with pedestrian-like appearance and task activities
        drawRoNUnit(ctx, unit, 0, 0, currentZoom, color, gameState.tick);
      });
      
      // Update and draw global particles (smoke, dust, sparks, etc.)
